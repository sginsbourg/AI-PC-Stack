...
:: Create package.json using simple method
...
(
echo // Simple AI Dashboard Server
...
echo   console.log('AI Dashboard: http://localhost:' + PORT);
echo });
) > "%DASHBOARD_DIR%\server.js"

:: Create very simple HTML file
...
echo ^</div^>
echo ^<div id="services"^>Loading...^</div^>
echo ^</div^>
echo ^<script^>
echo var socket = io();
...
(
echo @echo off
echo echo AI Stack Dashboard
echo cd /d "%%~dp0"
echo node server.js
echo pause
) > "%DASHBOARD_DIR%\start.bat"
...
echo ===============================================
echo.
echo Dashboard created in: %DASHBOARD_DIR%
echo.
